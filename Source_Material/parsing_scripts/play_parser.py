    ...
    try:
        ...
        if direct[1] != '':
            ...
            multi_dict_line.append(
                build_dialogue_dict(current_speaker, direct[1]))
...
def parse_enter_and_dialogue(previous_line, line):
    ...
    try:
        ...
        if direct_and_line[1] != '':
            current_speaker = previous_line['character']
            multi_dict_line.append(dir_text)
            multi_dict_line.append(build_dialogue_dict(
                current_speaker, direct_and_line[1]))
            return multi_dict_line
    except:
        pass
    ...
    for index, line in enumerate(lines):
        try:
            ...
            try:
                ...
                multi_dict_line = parse_direction_and_dialogue(
                    previous_line, line)
                ...
                try:
                    ...
                    multi_dict_line = parse_enter_and_dialogue(
                        previous_line, line)
                    for line_dict in multi_dict_line:
                        parsed_scene.append(line_dict)
                except:
...
def read_parse_dump_play(play_title):
    ...
    with open('Source_Material/plays/plays_parsed/{}.json'.format(play_title), 'w') as sp:
